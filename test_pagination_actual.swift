...
// Let's use indices of the layoutWeights values
let layoutWeights = [1.0, 0.5, 0.5, 0.5, 0.4, 0.4, 0.4, 0.4, 0.5, 0.6, 0.6]
// The actual layout types array mapped to weights:
...
let layoutSeq = [0.4, 0.4, 1.0, 0.4, 0.5, 0.4, 0.5, 0.6, 0.4, 0.4, 1.0]

let totalWeight = layoutSeq.reduce(0.0, +)
...
while currentSentenceIndex < sentences.count {
    let currentWeight = layoutSeq[pageIndex % layoutSeq.count]
    ...
    if pageIndex == layoutSeq.count - 1 {
        while currentSentenceIndex < sentences.count {
            let sentence = sentences[currentSentenceIndex]
            chunk += " " + sentence
            currentSentenceIndex += 1
        }
        print("Page \(pageIndex + 3) (FINAL REST): \(chunk)")
        break
    } else {
